...
#ifndef PROCESSTREE_TREE_H
#define PROCESSTREE_TREE_H
...
public:
  ...
  std::optional<process_tree::Annotations>
  ExportAnnotations(const struct Pid p);
  ...
  RootSlice(std::shared_ptr<const Process> p) const;
  ...
  std::optional<std::shared_ptr<const Process>>
  Get(const struct Pid target) const;
...
private:
  ...
  std::optional<std::shared_ptr<Process>>
  GetLocked(const struct Pid target) const ABSL_SHARED_LOCKS_REQUIRED(mtx_);
  ...
  absl::flat_hash_map<const struct Pid, std::shared_ptr<Process>>
      map_ ABSL_GUARDED_BY(mtx_);
...
template <typename T>
std::optional<std::shared_ptr<const T>>
ProcessTree::GetAnnotation(const Process &p) const {
...
absl::StatusOr<std::shared_ptr<ProcessTree>>
CreateTree(std::vector<std::unique_ptr<Annotator>> &&annotations);
