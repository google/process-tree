...
ProcessTree::ExportAnnotations(const Pid p) {
  ...
  process_tree::Annotations a;
  for (const auto &[_, annotation] : (*proc)->annotations_) {
    if (auto x = annotation->Proto(); x)
      a.MergeFrom(*x);
  }
  return a;
}
...
ProcessTree::RootSlice(std::shared_ptr<const Process> p) const {
...
ProcessTree::Get(const Pid target) const {
  absl::ReaderMutexLock lock(&mtx_);
  return GetLocked(target);
}

std::optional<std::shared_ptr<Process>>
ProcessTree::GetLocked(const Pid target) const {
